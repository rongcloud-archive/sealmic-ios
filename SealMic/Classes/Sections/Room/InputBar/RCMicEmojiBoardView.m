...
#define RC_IOS_SYSTEM_VERSION_LESS_THAN(v) ([[[UIDevice currentDevice] systemVersion] compare:v options:NSNumericSearch] == NSOrderedAscending)
...
@property(nonatomic, strong) NSMutableArray
    *tabIconArray; // tabIconArray 中包含的表情tab 按钮 比 emojiModelList 多1个，emojiModeList 中不包含emoji字符串表情
//
///**
...
@implementation RCMicEmojiBoardView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        CGFloat height = 186.5;
        self.emojiBackgroundView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, height)];
        self.emojiBackgroundView.backgroundColor = RCMicColor(HEXCOLOR(0xf8f8f8, 1.0), HEXCOLOR(0xf8f8f8, 1.0));
        self.emojiBackgroundView.pagingEnabled = YES;
        self.emojiBackgroundView.contentSize = CGSizeMake(self.emojiTotalPage * self.frame.size.width, height);
        ...
        self.backgroundColor = RCMicColor(HEXCOLOR(0xf8f8f8, 1.0), HEXCOLOR(0xf8f8f8, 1.0));
        [self loadLabelView];
    }
    return self;
}
...
- (void)generateDefaultLayoutParameters {
    ...
    if (nil == self.faceEmojiArray || [self.faceEmojiArray count] == 0) {
        ...
        self.emojiTotal = (int)[self.faceEmojiArray count]; // sizeof(faceEmojiArray) / sizeof(faceEmojiArray[0]);
                                                            // //(int)[self.faceEmojiArray count];//
    }
...
- (void)loadLabelView {
    ...
    if (!pageCtrl) {
        pageCtrl = [[RCMicEmojiPageControl alloc] initWithFrame:CGRectMake(0, 175, self.frame.size.width, 5)];
    ...
    _tabbarView = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - 36 - iphoneXHeight, self.frame.size.width, 36)];
    _tabbarView.backgroundColor = [UIColor whiteColor];
    [self addSubview:_tabbarView];
    self.sendBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    self.sendBtn.tag = 333;
    self.sendBtn.titleLabel.font = [UIFont systemFontOfSize:14.0f];
    self.sendBtn.frame = CGRectMake(self.frame.size.width - 52, 1, 52, 35);
    [self.sendBtn setTitle:NSLocalizedStringFromTable(@"Send", @"RongCloudKit", nil) forState:UIControlStateNormal];
    [self.sendBtn addTarget:self action:@selector(sendBtnHandle:) forControlEvents:UIControlEventTouchUpInside];
    [_tabbarView addSubview:self.sendBtn];
...
- (void)enableSendButton:(BOOL)sender {
    ...
    if (sender) {
        ...
        sendButton.backgroundColor = RCMicColor(HEXCOLOR(0x0099ff, 1.0), HEXCOLOR(0x0099ff, 1.0));
        [sendButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    } else {
        sendButton.userInteractionEnabled = NO;
        sendButton.backgroundColor = RCMicColor(HEXCOLOR(0xfafafa, 1.0), HEXCOLOR(0xfafafa, 1.0));
        [sendButton setTitleColor:RCMicColor(HEXCOLOR(0x999999, 1.0), HEXCOLOR(0x999999, 1.0)) forState:UIControlStateNormal];
    }
}
...
- (void)loadEmojiViewPartly {
    ...
    for (int i = beginEmojiBtn; i < endEmojiBtn; i++) {
        ...
        if (((i + 1) >= self.emojiMaxCountPerPage && (i + 1) % self.emojiMaxCountPerPage == 0) ||
            ...
            frame.origin.x = self.frame.size.width - startPos_X - 30 + pageIndex * self.frame.size.width;
            frame.size = CGSizeMake(RC_EMOJI_WIDTH, RC_EMOJI_WIDTH);
            deleteButton.frame = frame;
            [deleteButton setImage:[UIImage imageNamed:@"input_emoji_delete"]
...
- (void)loadCustomerEmoticonPackage {
    ...
    self.emotionTabView =
        [[UIScrollView alloc] initWithFrame:CGRectMake(0, 1, emotionTabViewWidth, 35)]; //暂时不显示加号，所以位置前移
    ...
    emojiBtn.backgroundColor = RCMicColor(HEXCOLOR(0xf8f8f8, 1.0), HEXCOLOR(0xf8f8f8, 1.0));
    emojiBtn.frame = CGRectMake(0, 0, 45, 35);
    [emojiBtn setImage:[UIImage imageNamed:@"input_emoji_item_normal"] forState:UIControlStateNormal];
...
- (void)tabBtnHandle:(UIButton *)sender {
    for (int i = 0; i < self.tabIconArray.count; i++) {
        ...
        if (sender == btn) {
            btn.backgroundColor = RCMicColor(HEXCOLOR(0xf8f8f8, 1.0), HEXCOLOR(0xf8f8f8, 1.0));
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    int curIndex = offset.x / bounds.size.width;
    pageCtrl.numberOfPages = self.emojiTotalPage;
    for (UIButton *btn in self.tabIconArray) {
        btn.backgroundColor = [UIColor clearColor];
    }
    UIButton *tabBtn = self.tabIconArray[0];
    tabBtn.backgroundColor = RCMicColor(HEXCOLOR(0xf8f8f8, 1.0), HEXCOLOR(0xf8f8f8, 1.0));
    pageCtrl.numberOfPages = self.emojiTotalPage;
    [pageCtrl setCurrentPage:curIndex];
